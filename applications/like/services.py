...
from applications.like.serializers import FanSerializer
...
def like_unlike(user, obj) -> str:
    ...
    :param obj: movie, которому ставят лайк
    :return: статус: like/unlike
    """
    like_obj, is_created = Like.objects.get_or_create(user=user, movie=obj)
...
def is_fan(user, obj) -> bool:
    ...
    """
    try:
        like = Like.objects.filter(user=user, movie=obj)
...
def get_fans(obj) -> List[dict]:
    ...
    :param obj: movie которому поставили лайк
    """
    fans = Like.objects.filter(movie=obj, like=True)
